...
static esp_err_t request_get_rele_status(httpd_req_t *req) {
	...
	int r1;
	esp_io_expander_get_level(io_expander, IO_EXPANDER_PIN_NUM_0,
							  (uint32_t *)&r1);
	int r2;
	esp_io_expander_get_level(io_expander, IO_EXPANDER_PIN_NUM_1,
							  (uint32_t *)&r2);
	int r3;
	esp_io_expander_get_level(io_expander, IO_EXPANDER_PIN_NUM_2,
							  (uint32_t *)&r3);
	int r4;
	esp_io_expander_get_level(io_expander, IO_EXPANDER_PIN_NUM_3,
							  (uint32_t *)&r4);
	int r5;
	esp_io_expander_get_level(io_expander, IO_EXPANDER_PIN_NUM_4,
							  (uint32_t *)&r5);
	int r6;
	esp_io_expander_get_level(io_expander, IO_EXPANDER_PIN_NUM_5,
							  (uint32_t *)&r6);
	int r7;
	esp_io_expander_get_level(io_expander, IO_EXPANDER_PIN_NUM_6,
							  (uint32_t *)&r7);
	int r8;
	esp_io_expander_get_level(io_expander, IO_EXPANDER_PIN_NUM_7,
							  (uint32_t *)&r8);
	...
	ESP_LOGI(TAG, "STATUS RELE %d %d %d %d %d %d %d %d", r1, r2, r3, r4, r5, r6,
			 r7, r8);

	sprintf(resp_str, "%d,%d,%d,%d,%d,%d,%d,%d", (int)r1, (int)r2, (int)r3,
			(int)r4, (int)r5, (int)r6, (int)r7, (int)r8);
...
httpd_handle_t start_webserver(void) {
	...
	// a privileged user in linux. So when a unprivileged user tries to run the
	// application, it throws bind error and the server is not started. Port
	// 8001 can be used by an unprivileged user as well. So the application will
